...
class NmapApp(QtWidgets.QWidget):
    ...
    def setupUi(self):
        ...
        self.target_label = QtWidgets.QLabel("Target IP Address or Range:")
    ...
    def start_scan(self):
        ...
        for target in targets:
            if not self.is_valid_ip_address_or_range(target):
                self.show_error_message(f"Invalid IP Address/Range: {target}")
                return
        ...
        scanner = NmapScanner(targets, nmap_commands, PostScanDialog, self)
        scanner.resultReceived.connect(self.display_result)
        scanner.progressUpdated.connect(self.update_progress)

        thread = threading.Thread(target=scanner.start_scan)
        thread.start()
    ...
    def is_valid_ip_address_or_range(self, ip_var):
        try:
            _ = ipaddress.ip_network(ip_var)
            return True
